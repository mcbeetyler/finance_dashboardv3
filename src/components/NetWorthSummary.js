...
// Top-level summary cards: assets, liabilities, net worth

import { formatUSD } from "../utils/currency";

export function NetWorthSummary({ totalAssets, totalLiabilities, fxStatus }) {
  ...
  return (
    <div style={{ marginBottom: "2rem" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "1rem" }}>
        <div>
          <p style={{ fontSize: "13px", color: "#888", margin: "0 0 4px" }}>Net worth</p>
        ...
        <span style={{
          ...
          {fxStatus === "live" ? "FX live" : fxStatus === "loading" ? "FX loading…" : "FX fallback"}
        </span>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "12px" }}>
        <MetricCard label="Total assets" value={formatUSD(totalAssets)} />
        <MetricCard label="Total liabilities" value={formatUSD(totalLiabilities)} negative />
